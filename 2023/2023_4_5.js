...
function findSenior(list) {
  const oldestDev = list.reduce(
    (acc, dev) => {
      if (dev.age > acc[0].age) {
        return [dev];
      } else if (dev.age === acc[0].age) {
        acc.push(dev);
      }
      return acc;
    },
