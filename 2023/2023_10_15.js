/* <1> https://www.codewars.com/kata/58de08d376f875dbb40000f1/train/javascript
...
function premierLeagueStandings(teamStandings) {
  let firstPlace = teamStandings[1];

  delete teamStandings[1];
  let otherTeams = Object.values(teamStandings).sort();
  console.log(otherTeams, "TEAMS ALPHABETIZED");
  let output = {};
  for (let i = 0; i < otherTeams.length; i++) {
    output[i + 2] = otherTeams[i];
  }

  return { 1: firstPlace, ...output };
}
