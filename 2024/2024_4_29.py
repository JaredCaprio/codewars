...
def frog_contest(flies):
    ...
    chris = gauss_summation(flies)
    tom = gauss_summation(math.floor(chris / 2))
    cat = int(gauss_summation(chris + tom))

    return f"Chris ate {int(chris)} flies, Tom ate {int(tom)} flies and Cat ate {cat} flies"
